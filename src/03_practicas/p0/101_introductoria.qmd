...
- Intérprete de Python: [Python](https://www.python.org/)
...
- Descarga el instalador de Python desde la [página oficial](https://www.python.org/).
...
![instalacion-vsc-fin](img/12b-conf-vsc-ocultar-copilot.png)
...
Os recomendamos seguir una estructura concreta para tener todo el material de prácticas organizado.

Agruparemos los archivos de código fuente de cada práctica en su propio directorio (carpeta).

:::
::: {.column width="50%"}

![](img/25-sistema-ficheros.png)
...
Os vamos a dar la estructura básica de carpetas junto con unos archivos de configuración de Visual Studio Code que os van a facilitar **hacer pruebas** de los programas que escribáis. 

La estructura básica la tenéis en [`practicas_fiib.zip`](./practicas_fiib.zip).

Las pruebas de esta práctica 0, en [`tests_p0.zip`](./tests_p0.zip).

---

**Tarea:** Crea la estructura de carpetas en tu sistema de ficheros utilizando el Explorador de archivos o Finder.

En el caso de Windows:

1. Crea una carpeta para almacenar los archivos de esta asignatura
    - Puedes llamarla, por ejemplo, `FIIB`.
    - Créala en un lugar adecuado, como la carpeta `Documentos`.
    - `Clic derecho` > `Nuevo` > `Carpeta` > `FIIB`
1. Accede a la carpeta que has creado y déjala abierta
1. Descarga el fichero [`practicas_fiib.zip`](./practicas_fiib.zip).
1. Abre el archivo `practicas_fiib.zip` que te has descargado.
1. Arrastra todo el contenido del archivo `practicas_fiib.zip` a la carpeta `FIIB`.

---

![](img/26-arrastrar-carpetas-practicas.png)

---

**Tarea:** Copia los archivos de pruebas de la práctica 0.

1. En tu carpeta de prácticas `FIIB`, accede a la carpeta `tests` de la carpeta `p0` y déjala abierta.
1. Descarga el fichero [`tests_p0.zip`](./tests_p0.zip).
1. Abre el archivo `tests_p0.zip` que te has descargado.
1. Arrastra todo el contenido del archivo `tests_p0` a la carpeta `tests` de la carpeta `p0`.
...
![](img/27-arrastrar-archivos-pruebas.png)
...
- Forma 2:
    ...
    1. `Open Folder` > Selecciona la carpeta `p0`
    1. Confirma que confías en el directorio
