...
**Tarea:** Crea la estructura de directorios en tu sistema de ficheros utiliizando el `Explorador de Windows`.

1. `Click derecho` > `Nueva` > `Carpeta` > `p0`
1. Accede a la carpeta `p0`
1. `Click derecho` > `Nuevo` > `Documento de texto` > `saludo.py`
    1. Asegúrate de que la extensión del fichero es `.py`

_Nota:_ Algunas versiones de Windows oculan por defecto las extensiones de los ficheros. Puedes mostrarlas en `Vista` > `Mostrar u ocultar` > `Extensiones de nombre de archivo`.
...
- Forma 1:
    ...
    1. `Click derecho` > `Abrir directorio en VSCode`
...
El arbol de ficheros muestra la estructura de directorios y ficheros del proyecto.
...
1. Abre el fichero `hello.py` desde el arbol de ficheros
...
- Dicha interacción se realiza tamibién en la propia **terminal**.
...
1. Utiliza el arbol de ficheros para abrir el fichero `saluda.py`
...
1. Abrelos con el editor de texto y lee su contenido.
...
_Nota:_ Puedes hacer `Click derecho` > `Guardar enlace como...` en los nombres de cada fichero para descargarlos o hacer click en el `icono del portapapeles` para copiar todo el contenido.
...
- [ ] Verifica que has añadido un _docstring_ a cada fichero son tu nombre y NIP.
...
- [ ] Sube los ficheros `*.py` individualmente (no comprimidos).

_Nota:_ No se aceptarán entregas fuera de plazo. En el caso de que se acerque la _deadline_, se recomienda al menos subir los ejercicios que se hayan terminado. **Se podrán resubir** tantas veces como se desee antes de la _deadline_.
