...
- Una función denominada `str_complejo()` que, dado un dato de tipo `Complejo` devuelva un string con la representación del número en forma binomial ($a+bi$) siendo dos reales con 4 decimales máximos para cada componente.
...
- Tres funciones denominadas `mayor_que()`, `menor_que()` e `igual_que()` que, dados dos números complejos, devuelvan `True` si se cumple que el módulo del primero sea mayor/menor/igual que el módulo del segundo.
...
```python
print(str_complejo(Complejo(3.0, 4.0)))
```
...
```python
print(str_complejo(sumar(Complejo(3.0, 4.0), Complejo(1.0, 2.0))))
```
...
Copia los archivos de pruebas de la práctica 4 en la carpeta `test` de la carpeta `p5` que creaste con la estructura básica de carpetas en la [práctica 0](../p0/101_introductoria.html#/22):

1. En tu carpeta de prácticas `FIIB`, accede a la carpeta `tests` de la carpeta `p5` y déjala abierta.
1. Descarga el fichero [`tests_p5.zip`](./tests_p5.zip).
1. Abre el archivo `tests_p5.zip` que te has descargado.
1. Arrastra todo el contenido del archivo `tests_p5.zip` a la carpeta `tests` de la carpeta `p5`.
