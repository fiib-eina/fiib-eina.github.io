...
- Puedes obtener el carácter correspondiente a un valor numérico utilizando la función `chr()` (carácter).
    - `chr(65)` devuelve `A` y `chr(97)` devuelve `a`.
:::
...
Copia los archivos de pruebas de la práctica 4 en la carpeta `test` de la carpeta `p6` que creaste con la estructura básica de carpetas en la [práctica 0](../p0/101_introductoria.html#/22):
 
1. En tu carpeta de prácticas `FIIB`, accede a la carpeta `tests` de la carpeta `p6` y déjala abierta.
1. Descarga el fichero [`tests_p6.zip`](./tests_p6.zip).
1. Abre el archivo `tests_p6.zip` que te has descargado.
1. Arrastra todo el contenido del archivo `tests_p6.zip` a la carpeta `tests` de la carpeta `p6`.
