...
#### Ejercicio: Conjugar correctamente
...
- Extraemos la lógica de la **conjugación** de palabras a una función `conj()`.

::: {.columns}
::: {.column width="50%"}

```python
def conj(num, palabra):
...
def num_manzanas(n):
    return f"Tienes {n} {conj(n, 'manzana')}"

def num_barcos(n):
    return f"Tienes {n} {conj(n, 'barco')}"
